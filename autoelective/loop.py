...
def _get_refresh_interval():
    ...
    delta = (random.random() * 2 -
             1) * refresh_random_deviation * refresh_interval
        ...
        try:
                ...
                while True:
                    ...
                    try:
                        ...
                        _ = elective.get_SupplyCancel(
                            username)  # 遇到空页面时请求一次补退选主页，之后就可以不断刷新
                    else:
                        break
                    finally:
                        retry -= 1
                        ...
                        if c0 == c:
                            if c0.is_available():
                                ...
                                if delay != NO_DELAY and c0.remaining_quota > delay:
                                    ...
                                    # TODO: use server jiang to send a message
                                    # to notify user

                                    send_key = config.server_chan_key
                                    if send_key:
                                        import requests
            ...
            while len(tasks) > 0:
                ...
                while True:
                    ...
                    url = 'http://api.ttshitu.com/api/captcha/'
                    ...
                    def base64_api(uname, pwd, img, typeid):
                        ...
                        result = json.loads(
                            requests.post("http://api.ttshitu.com/predict",
                                          json=data).text)
                        if result['success']:
                    ...
                    ttshitu_username = config.ttshitu_username
                    ttshitu_password = config.ttshitu_password
                    result = base64_api(ttshitu_username, ttshitu_password,
                                        r.content, 7)
                    cout.info("Captcha result: %s" % result)

                    # TODO: user an online api to valid captcha
                    # captcha = recognizer.recognize(r.content)
                    # cout.info("Recognition result: %s" % captcha.code)

                    r = elective.get_Validate(username, result)
                    try:
